...
describe ThirdSocialMediaController do
	...
	let(:stubbed_sign_in_response){
		...
		allow(stubbed_sign_in_response).to receive(:body).and_return("my_token")
	...
	it "should 	export my data to JSON format when I enter my credentials" do
		...
		get :export, user: "my_user@me", passowrd: "my_password"
		expect(response.body).to start_with "{\"count\":"
	end
end